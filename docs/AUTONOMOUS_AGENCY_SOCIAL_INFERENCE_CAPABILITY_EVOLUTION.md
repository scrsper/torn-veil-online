...
Encounter records keep at most eight observations. Repeated calls in one tick cannot build familiarity. Ordinary observations are sampled no faster than 60 world seconds (300 for lightweight minds), and continuous observation stops producing encounter memories after three retained samples until a meaningful absence or changed signature. The latest seen location/activity can still update without another historical event. Each retained encounter uses an actual `perceived` event and an ordinary KMSI memory.
...
Spatial neighborhoods are reused; no global rumor broadcast or all-pairs social census is introduced. Additional cognition is bounded by a person's existing knowledge budget and local percept count. Social decision inputs use direct lookup of eight relevant belief keys. New inference runs on acquired evidence; continuous observation stops allocating encounter memories once sufficient familiarity exists. Appearance samples occur at most every five world seconds (fifteen for lightweight minds); retained familiarity samples are at least sixty/three hundred seconds apart. Developer inspection intentionally scans historical events and is not a per-tick gameplay path.
