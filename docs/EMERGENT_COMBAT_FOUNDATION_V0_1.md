...
## Family trace fixture repair

Compared the unmodified family trace at seed 606060 against parent `4bc4661` (all six checks pass) and injury HEAD `e838f82` (only the multi-step check fails). The warm-up combat history changes under legitimate injury effects. Neither selected spouse has regional injuries at setup, but Jory is hauling at (86.63, 58.71) rather than sleeping at (111.5, 107.5). Greta is at (116.43, 84.44) rather than (94.55, 95.80): about 39 metres from the assault instead of 21, outside the attack's 26-metre visibility. Nell's separation also grows from 25 to 39 metres. This changes the report opportunity, not her ability to adopt family goals.

In the parent, Greta tells Nell on day 100 around 19.2h; Nell adopts check_on, provide and help while Jory still needs assistance (30 adoptions total). With injuries, Nell first learns through Jory on day 101 around 10.6h, adopts provide once at 10.8h, then resolves the concern as seen_well. Requiring a multi-step response without ensuring timely information was the brittle prerequisite.

The family fixture now stages a structurally selected trusted messenger beside the victim, uses the victim's actual witnessed knowledge in `Simulation.tell`, and relays that acquired knowledge to the separated spouse through the same canonical method. Both tell events preserve provenance; no concern, purpose or goal is assigned. Autonomous behavior resumes with the same seed, observation window and all original assertions. The fixture also now snapshots the aggressor's position before staging the assault, matching its existing promise to return them afterward. Production society and injury rules are unchanged. This repair supersedes the outstanding family-trace failure recorded above.
