...
TSharedRef<SWidget> UTVDialogueWidget::RebuildWidget(){Body=WidgetTree->ConstructWidget<UVerticalBox>();auto* Scroll=WidgetTree->ConstructWidget<UScrollBox>();Scroll->SetScrollWhenFocusChanges(EScrollWhenFocusChanges::InstantScroll);Scroll->AddChild(Body);WidgetTree->RootWidget=WrapModal(WidgetTree,Scroll);Rebuild();return Super::RebuildWidget();}
