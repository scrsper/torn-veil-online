...
void UTVDialogueWidget::Rebuild(){if(!Body)return;if(!SpeakerText||ChoiceButtons.Num()!=Snapshot.DialogueOptionLabels.Num()||LineTexts.Num()!=Snapshot.DialogueLines.Num()){Body->ClearChildren();ChoiceButtons.Reset();LineTexts.Reset();SpeakerText=MakeText(WidgetTree,Snapshot.DialogueSpeaker+TEXT(" / ")+Snapshot.DialogueOccupation,24);Body->AddChildToVerticalBox(SpeakerText);for(const FString& L:Snapshot.DialogueLines){auto* T=MakeText(WidgetTree,L);LineTexts.Add(T);Body->AddChildToVerticalBox(T);}for(int32 I=0;I<Snapshot.DialogueOptionLabels.Num();++I)AddChoice(I,Snapshot.DialogueOptionIds.IsValidIndex(I)?Snapshot.DialogueOptionIds[I]:FString(),Snapshot.DialogueOptionLabels[I]);Body->AddChildToVerticalBox(MakeButton(WidgetTree,CommandDelegate,ETVUICommand::Back,FString(),FString(),INDEX_NONE,TEXT("Close conversation  [Esc / B]")));}else{SpeakerText->SetText(FText::FromString(Snapshot.DialogueSpeaker+TEXT(" / ")+Snapshot.DialogueOccupation));for(int32 I=0;I<LineTexts.Num();++I)LineTexts[I]->SetText(FText::FromString(Snapshot.DialogueLines[I]));}for(int32 I=0;I<ChoiceButtons.Num();++I){ChoiceButtons[I]->Configure(CommandDelegate,ETVUICommand::DialogueChoice,Snapshot.DialogueOptionIds.IsValidIndex(I)?Snapshot.DialogueOptionIds[I]:FString(),FString(),I);ChoiceButtons[I]->SetLabel(FString::Printf(TEXT("%d  %s"),I+1,*Snapshot.DialogueOptionLabels[I]));}}
