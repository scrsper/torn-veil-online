...
describe('player progression journal', () => {
  it('shows earned practice and martial-family technique knowledge without revealing an unknown teacher name', () => {
    ...
    for (const id of ATTRIBUTE_IDS) learner.attributes[id] = 7;
    learner.physiology.fatigue = .7;
    ...
    // Recovery plus practice and seven foundations exceed the former eight-line truncation.
    expect(journal.advancement.remaining).toHaveLength(9);
    expect(journal.advancement.remaining).toContain('will 7 must reach 11');
  });
});
