import { repertoireTransition } from './combatRepertoire';
...
export function combatTransitionAt(a: CombatAction | undefined, next: CombatInput['kind']|'move'): number {
  if (!a) return -Infinity;
  if (a.outcome === 'interrupted' || a.outcome === 'cancelled') return a.completeAt;
  const authored=repertoireTransition(a,next);if(authored!==undefined)return Math.min(a.completeAt,authored);
  if(next==='move'&&a.repertoireRevision!==1)return a.completeAt;
