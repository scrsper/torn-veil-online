...
export const genealogicalBeliefs = (p: Person): KnowledgeItem[] => knowledgeItems(p).filter(k => k.claim.genealogy);
...
export function inferGenealogy(world: World, p: Person): void {
  if (p.age < 3) return;
  inferKnownGenealogy(world, p, genealogicalBeliefs(p));
}

/** Return whether inference attempted a write; callers must refresh after learning/pruning. */
function inferKnownGenealogy(world: World, p: Person, known: KnowledgeItem[]): boolean {
  if (p.age < 3) return false;
  const edges = known.filter(k => ['parent', 'ancestor'].includes(k.claim.genealogy.relationship));
  ...
  for (const first of edges.filter(k => k.claim.genealogy.subjectId === p.id)) {
    ...
    for (const second of bySubject.get(a.relativeId) ?? []) {
      if (++inspected > 32) return added > 0;
...
export function genealogyGoals(world: World, p: Person): Partial<Goal>[] {
  let known = genealogicalBeliefs(p);
  // Reuse the fresh traversal only when inference made no writes. Learning may prune or
  // refine this collection, so a changed mind must be read again before offering testimony.
  if (inferKnownGenealogy(world, p, known)) known = genealogicalBeliefs(p);
