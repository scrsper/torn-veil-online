...
export class DialogueSystem {
  ...
  private trade(npc: Person, player: Person): DialogueState {
    ...
    const opts: DialogueOption[] = goods.slice(0, 8).map(it => { const price = Math.max(1, Math.round(it.value * markup)); return { label: `Buy ${it.name} (${price}s)`, next: () => { if (!coins || coins.quantity < price) { return { speaker: npc, lines: [`You haven't the coin.`], options: this.options(npc, player) }; } coins.quantity -= price; npc.wealth += price; this.sim.takeItem(player, it, 'bought', npc.id); it.ownerId = player.id; adjustRel(w, npc, player.id, { affection: 0.05, trust: 0.05 }, 'traded'); return { speaker: npc, lines: [`Done. ${npc.traits.greed > 0.7 ? 'Pleasure doing business.' : 'Fair price.'}`], options: this.options(npc, player) }; } }; });
    // sell
    for (const id of player.inventory) { const it = w.item(id); if (!it || it.type === 'coins' || it.type === 'dagger') continue; if (it.ownerId && it.ownerId !== player.id && npc.knowledge[`owner:${it.id}`]) continue; const price = Math.max(1, Math.round(it.value * 0.5 / markup)); opts.push({ label: `Sell ${it.name} (${price}s)`, next: () => { if (coins) coins.quantity += price; else { /* create coins */ } npc.wealth -= price; player.inventory = player.inventory.filter(x => x !== it.id); it.holderId = null; it.ownerId = npc.id; const spot = w.place(npc.workId)?.anchors.find(a => a.kind === 'display'); it.pos = spot ? { x: spot.pos.x + 0.5, y: spot.pos.y, z: spot.pos.z + 0.5 } : { ...w.primaryBody(npc.id)!.pos }; it.placeId = npc.workId; it.provenance.push({ tick: w.now, from: player.id, to: npc.id, how: 'sold' }); w.emit('trade', { actor: player.id, target: npc.id, item: it.id, significance: 0.15, summary: `the Traveler sold ${it.name} to ${npc.name}` }); return { speaker: npc, lines: [`I'll take it.`], options: this.options(npc, player) }; } }); }
