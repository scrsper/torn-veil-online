...
export function makeBody(world: World, ownerId: EntityId, pos: Vec3, shape: Body['shape'] = 'humanoid', maxHealth = 80): Body {
  const b: Body = {
    ...
    path: null, pathIndex: 0, pathGoal: null, speed: 3.4, health: maxHealth, maxHealth, dead: false, lastHitAt: -99, lastAttackAt: -99, sitAnchor: null, present: true,
  };
...
export function makePerson(world: World, s: PersonSpec): Person {
  ...
  const p: Person = {
    id: world.nextId('p'), kind: 'person', name: s.name, createdAt: world.now - s.age * 365 * 86400, tags: s.tags ?? [],
    ...
    schedule: [], bio: s.bio, alive: true, controlled: false, desires: [], hostile: !!s.hostile, speech: null,
  };
...
export function makePlace(world: World, type: PlaceType, name: string, bounds: Place['bounds'], o: { door?: Vec3 | null; inside?: Vec3; anchors?: Anchor[]; owner?: EntityId | null; description?: string; indoor?: boolean; parent?: EntityId | null; fires?: Vec3[]; chimneys?: Vec3[]; tags?: string[] } = {}): Place {
  const p: Place = {
    id: world.nextId('pl'), kind: 'place', name, createdAt: world.now - 20 * 365 * 86400, tags: o.tags ?? [], type, bounds, door: o.door ?? null,
...
export function makeFaction(world: World, name: string, description: string): Faction {
  const f: Faction = { id: world.nextId('f'), kind: 'faction', name, createdAt: world.now - 50 * 365 * 86400, tags: [], members: [], description, hostileTo: [] };
