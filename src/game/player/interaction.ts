...
export class Interaction {
  ...
  attack(): void {
    ...
    if (t?.kind === 'body' && t.dist < 3.2) { const dmg = Math.max(8, this.sim.weaponOf(this.player) || 8) * (0.85 + Math.random() * 0.3); this.sim.applyHit(this.player, pb, t.body, dmg); }
  }
  interact(): void {
