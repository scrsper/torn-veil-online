...
No backend, no external services, no LLM API calls — everything (world generation,
NPC cognition, dialogue) runs client-side and deterministically from a seed.
...
| Left-click | attack with whatever you're holding |
...
- **Entities, not sprites.** Every person, item, and place is a persistent entity with an id
  ...
  entity — the ontology doesn't assume one entity has exactly one body.
...
- **Perception** (`src/sim/mind/agent.ts`): line-of-sight sight cones and loudness-radius
  ...
  grid), daylight, and whether the NPC is asleep.
- **Knowledge with provenance** (`src/sim/mind/knowledge.ts`): every belief an NPC holds is
  ...
  plus a confidence and a hop count. Gossip decays in reliability as it spreads.
...
- **Persistence** (`src/sim/persist/save.ts`): the village regenerates deterministically from
  its seed, then saved state (minds, relationships, memories, knowledge, items, voxel edits,
  event log) is overlaid on top, so consequences survive a reload.
...
src/sim/            canonical simulation — no rendering concerns
  ...
  physical/          voxel grid, block palette, A* navigation
...
the World every frame; it never owns state the simulation needs.
